...
class Slab:
    ...
    def from_bulk_get_specific_millers(
        ...
        if save_path is not None:
            ...
            slabs = [
            (
                tile_and_tag_atoms(s[1] if s[1] is not None else s[0], bulk.atoms, min_ab=min_ab),
                s[3], # millers
                s[4], # shift
                s[5], # top
                s[0], # oriented_unit_cell (keep for metadata)
            )
...
def tile_and_tag_atoms_with_info(
    unit_slab_struct: Structure, bulk_atoms: ase.Atoms, min_ab: float = 8
):
    """
    This function combines the next three functions that tile, tag,
    and constrain the atoms, returning repeat information.
    ...
    Returns
    -------
    tuple: (atoms_tiled, na, nb)
        atoms_tiled: ase.Atoms
            A copy of the slab atoms that is tiled, tagged, and constrained
        na: int
            Number of repeats in x-direction (a vector)
        nb: int
            Number of repeats in y-direction (b vector)
    """
    slab_atoms = AseAtomsAdaptor.get_atoms(unit_slab_struct)
    tagged_atoms = tag_surface_atoms(slab_atoms, bulk_atoms)
    tiled_atoms, na, nb = tile_atoms_with_info(tagged_atoms, min_ab=min_ab)
    return set_fixed_atom_constraints(tiled_atoms), na, nb
...
def tile_atoms_with_info(atoms: ase.Atoms, min_ab: float = 8):
    """
    Repeat atoms structure in the direction of the a and b lattice vectors
    and return tiled structure with repeat information.
