"""
LMDB 파일과 mapping 파일로부터 모든 정보를 추출하여 test.csv 파일에 저장하는 스크립트
추출하는 정보:
- sid
...
def get_all_data_from_lmdb(lmdb_path: str, index: int) -> dict | None:
    ...
    try:
        ...
        ads_pos_initial = pos_np[adsorbate_mask] if np.any(adsorbate_mask) else None
        ...
        return {
            ...
            'ads_pos_initial': ads_pos_initial,
        }
...
def process_row(args):
    ...
    try:
        ...
        result = {
            ...
            'ads_pos_initial': str(lmdb_data['ads_pos_initial'].tolist()) if lmdb_data['ads_pos_initial'] is not None else None,
        }
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract all data from LMDB and mapping file to create test.csv"
    )
    parser.add_argument(
        "--lmdb-path",
        type=str,
    ...
    columns = [
        ...
        'true_tags', 'adsorbate_atomic_numbers', 'ads_pos_initial'
    ]
    ...
    non_null_ads_pos_initial = results_df['ads_pos_initial'].notna().sum()
    ...
    print(f"  Successfully extracted ads_pos_initial: {non_null_ads_pos_initial}")
